-- Phase 14: keep pull-from-e-Dnevnik sync compatible with existing sync action enum.
-- Existing enum values include UPDATE_STUDENT, so the reverse sync records its
-- direction in payload instead of requiring a new enum value.
